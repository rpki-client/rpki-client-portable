...
for i in explicit_bzero.c memmem.c memrchr.c strlcpy.c strlcat.c; do
...
for i in as.c asn1_bit_string.c aspa.c ccr.c cert.c cms.c constraints.c \
	...
	output-bird.c output-csv.c output-json.c output-ometric.c output.c \
	parser.c print.c repo.c rfc3779.c roa.c rpki-asn1.h rpki-client.8 \
	rrdp.c rrdp.h rrdp_delta.c rrdp_notification.c rrdp_snapshot.c \
	rrdp_util.c rsc.c rsync.c spl.c tak.c tal.c validate.c version.h \
	x509.c
do
	file=`basename ${i}`
	echo Copying ${file}
